...
iofit = ioglm((DepDelay > 15) ~ Distance + UniqueCarrier, data=data, family=binomial)
lmfit =   glm((DepDelay > 15) ~ Distance + UniqueCarrier, data=df  , family=binomial)
...
expect_equal(summary(iofit)[c("sigma","df","r.squared","adj.r.squared","fstatistic")],
             summary(lmfit)[c("sigma","df","r.squared","adj.r.squared","fstatistic")],tol=TOL)

# Run a basic glm model with a poisson loss function and check the terms
iofit = ioglm((DepDelay > 15) ~ Distance + UniqueCarrier, data=data, family=poisson)
lmfit =   glm((DepDelay > 15) ~ Distance + UniqueCarrier, data=df  , family=poisson)
expect_equal(coef(iofit),coef(lmfit),tol=TOL)
expect_equal(summary(iofit)$coefficients, summary(lmfit)$coefficients,tol=TOL)
expect_equal(summary(iofit)[c("sigma","df","r.squared","adj.r.squared","fstatistic")],
             summary(lmfit)[c("sigma","df","r.squared","adj.r.squared","fstatistic")],tol=TOL)
...
iofit = ioglm((DepDelay > 15) ~ Distance + UniqueCarrier - 1, data=data, family=binomial)
lmfit =   glm((DepDelay > 15) ~ Distance + UniqueCarrier - 1, data=df  , family=binomial)
expect_equal(coef(iofit),coef(lmfit),tol=TOL)
expect_equal(summary(iofit)$coefficients, summary(lmfit)$coefficients,tol=TOL)
expect_equal(summary(iofit)[c("sigma","df","r.squared","adj.r.squared","fstatistic")],
             summary(lmfit)[c("sigma","df","r.squared","adj.r.squared","fstatistic")],tol=TOL)
...
iofit = ioglm((DepDelay > 15) ~ Distance + UniqueCarrier, data=data, family=binomial, subset="DepDelay > 0")
lmfit =   glm((DepDelay > 15) ~ Distance + UniqueCarrier, data=df  , family=binomial, subset=df$DepDelay > 0)
...
iofit = ioglm((DepDelay > 15) ~ Distance + UniqueCarrier, data=data, family=binomial, weight="Month")
lmfit =   glm((DepDelay > 15) ~ Distance + UniqueCarrier, data=df,   family=binomial, weight=df$Month)
expect_equal(coef(iofit),coef(lmfit),tol=TOL)
expect_equal(summary(iofit)$coefficients, summary(lmfit)$coefficients,tol=TOL)
expect_equal(summary(iofit)[c("sigma","df","r.squared","adj.r.squared","fstatistic")],
             summary(lmfit)[c("sigma","df","r.squared","adj.r.squared","fstatistic")],tol=TOL)
...
iofit = ioglm((DepDelay > 15) ~ Distance + UniqueCarrier, data=data, family=binomial, offset="ArrDelay")
lmfit =   glm((DepDelay > 15) ~ Distance + UniqueCarrier, data=df,   family=binomial, offset=df$ArrDelay)
expect_equal(coef(iofit),coef(lmfit),tol=TOL)
expect_equal(summary(iofit)$coefficients, summary(lmfit)$coefficients,tol=TOL)
expect_equal(summary(iofit)[c("sigma","df","r.squared","adj.r.squared","fstatistic")],
             summary(lmfit)[c("sigma","df","r.squared","adj.r.squared","fstatistic")],tol=TOL)
